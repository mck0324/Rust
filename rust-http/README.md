...
--------------------------------------------------------------------------------------------------------------------------------------------------------
HashMap
RUST컴파일러는 src에 디렉터리에 아래에 있는 모든 파일을 컴파일링하지 않음. => 우린 이 파일들을 명시적으로 연결해줘야함 => 그래서 src/mod.rs파일이 존재하는것

// a=1&b=2&c&d=&e===&d=7&d=abc 같은 경우
d의 값이 3개이므로 쿼리 문자열에 어레이를 지정하는 방법은 여러가지가 있다.
pub enum Value {
    Single(&str),
    Multiple([]),
}
로 담으려고 하는데 배열 같은 경우 미리 배열의 길이를 선언해줘야 하기에 역동적으로 커질 수 있도록 힙을 사용해서 어레이를 할당해야함.예를 들어 우린 2개의 원소에 대한 공간을 할당 할 수 있다.하지만 우리가 세번쨰 원소를 넣으려고 할떄 어레이가 힙에서 증가해서 더 많은 데이터를 저장할 수 있음
RUST에서 힙 할당 동적 어레이를 Vector라고 하고 구조체의 이름은 Vec임
Vec는 제네릭하며 그리고 이게 저장하는 타입이 될것이다.